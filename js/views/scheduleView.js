...
export const initScheduleView = () => {
    ...
    view.innerHTML = `
        ...
        <section class="schedule-global-config panel-control">
            ...
            <form id="block-config-form" class="form-grid"><label class="form-field">Duración del bloque (minutos)<input name="blockDuration" type="number" min="5" max="240" step="5" value="${settings.blockDuration}" required></label><label class="form-field">Descanso mínimo — ${escapeHtml(activeCategory.nombre)}<input name="restBlocks" type="number" min="0" max="20" value="${DataManager.getCategoryRestBlocks(activeCategoryId)}" required><small>Bloques completos libres entre partidos de cada equipo.</small></label><div class="form-actions"><button class="btn-primary" type="submit">Guardar reglas</button></div></form>
        </section>
        <section class="schedule-conflicts ${conflicts.length ? 'has-conflicts' : ''}" aria-live="polite"><div><h3>Estado del fixture</h3><p>${conflicts.length ? `${conflicts.length} situación${conflicts.length === 1 ? '' : 'es'} requiere${conflicts.length === 1 ? '' : 'n'} atención.` : 'No se detectaron conflictos logísticos.'}</p></div>${conflicts.length ? `<ul>${conflicts.slice(0, 8).map(issue => `<li>⚠ ${escapeHtml(issue.message)}</li>`).join('')}</ul>` : '<span class="match-status finished">LISTO</span>'}</section>
    ...
    const card = match => { const parts = categoryParts(category(match.categoriaId)); return `<article class="fixture-match" data-match-id="${match.id}"><div class="fixture-match-slot"><strong>${escapeHtml(match.hora || 'Sin horario')}</strong><span>${escapeHtml(match.cancha || 'Sin cancha')}${match.orden ? ` · Orden ${match.orden}` : ''}</span></div><div class="fixture-match-context"><span class="fixture-category-label">${escapeHtml(parts.age)}${parts.modality ? ` · ${escapeHtml(parts.modality)}` : ''}</span><span>${escapeHtml(PHASE_LABELS[matchPhase(match)] || matchPhase(match))}${match.zonaId ? ` · ${escapeHtml(zoneName(match.zonaId))}` : ''}</span><div class="fixture-teams"><strong>${escapeHtml(teamName(match.equipoLocalId))}</strong><b>vs</b><strong>${escapeHtml(teamName(match.equipoVisitanteId))}</strong></div></div><span class="match-status ${match.estado === 'finalizado' ? 'finished' : 'pending'}">${escapeHtml(STATUS_LABELS[match.estado] || match.estado)}</span>${editor(match)}</article>`; };
    let viewMode = 'day';
    ...
    view.querySelector('#block-config-form').addEventListener('submit', event => { event.preventDefault(); try { const values = new FormData(event.currentTarget); DataManager.setTournamentSchedulingSettings(tournamentId, settings.duracionPartido, settings.intervaloPartidos, values.get('blockDuration')); DataManager.setCategoryRestBlocks(tournamentId, activeCategoryId, values.get('restBlocks')); initScheduleView(); } catch (error) { alert(error.message); } });
    ...
    view.querySelector('#btn-generar-programacion').addEventListener('click', () => { try { const result = LogisticsService.programTournament(tournamentId); const warning = result.failures.length ? `\n${result.failures.length} partido(s) quedaron sin programar: ${result.failures.map(item => item.reasons.join(', ')).join('; ')}.` : ''; alert(`${result.scheduled} partido(s) programado(s).${warning}`); initScheduleView(); } catch (error) { alert(error.message); } });
    ...
    view.querySelector('#export-fixture-full').addEventListener('click', () => downloadFixturePdf(tournament, exportRows({}), 'completo'));
    view.querySelector('#export-fixture-day').addEventListener('click', () => { const date = view.querySelector('#fixture-filter-date').value; if (!date || date === '__none') return alert('Seleccioná un día en los filtros para exportarlo.'); downloadFixturePdf(tournament, exportRows({ date }), date); });
